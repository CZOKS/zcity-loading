...
v.volume=.10;try{var old=localStorage.getItem('madness-volume');if(old!==null&&isFinite(Number(old)))v.volume=Math.max(0,Math.min(1,Number(old)));}catch(e){}updateVolume();
...
window.GameDetails=function(name,url,map,max,steam,mode,volume){$('server').textContent=name||'ZCity Madness';$('map').textContent=map||'—';$('status').textContent='Подключение к серверу';if(!volumeTouched&&volume!==undefined&&volume!==null&&isFinite(Number(volume))){/* Используем громкость сайта и выбор игрока. */updateVolume();}};
